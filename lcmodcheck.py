...
def get_mods(mods_file: str) -> list[LCMod]:
    ...
    for mod in mods_yaml:
        ...
        if mod_name is None or mod_author is None or mod_url is None:
            logging.warning(f"Mod {mod_name} is missing either a mod name, author or url. Skipping checks on it...")
            continue
        ...
        if mod_has_dlls(profile_dir, mod_name):
            ...
            mods.append(LCMod(name=mod_name, author=mod_author, url=mod_url, source_url=None))
    return mods

async def gather_tasks_in_batches(tasks: list[asyncio.Task], delay: float = 1.0, batch_size: int = 5) -> list:
    results = []
        ...
        for i in range(10):
            async with session.get(mod.url) as mod_page_response:
                if mod_page_response.status == 429:
                    logging.debug(f"Got status 429 for mod page: {mod.url}. Retrying in 1 second...")
                    await asyncio.sleep(1.0)
                    continue
                if mod_page_response.status != 200:
                    raise Exception(f"Got non-zero status code: {mod_page_response.status}")
                github_source_urls: list[str] = GITHUB_RE.findall((await mod_page_response.content.read()).decode())
                for url in github_source_urls:
                    ...
                    for blacklisted in GITHUB_BLACKLISTED_URLS:
                        ...
                        return LCMod(name=mod.name, author=mod.author, url=mod.url, source_url=url)
...
def mod_has_dlls(profile_dir: str, mod_name: str) -> bool:
    ...
    if not os.path.exists(mod_dir):
        raise Exception(f"Mod {mod_name} is missing from your r2modman profile. Please ensure it is installed")
...
def get_scan_exclusions():
    exclusions_file = os.path.join(os.path.dirname(os.path.realpath(__file__)), "scan_exclusions.txt")
    ...
    if not os.path.exists(exclusions_file):
        with open(exclusions_file, "w") as f:
            ...
            f.write("# Enter the name of each mod to exclude from scanning on their own lines below\nBepInEx-BepInExPack\n")
    if not os.path.exists(exclusions_file):
...
async def main():
    ...
    logging.info("Done checking mods. See the list of mods with invalid or missing github urls below")
    for mod in mods:
